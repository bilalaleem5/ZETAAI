...
2. Cite sources inline where relevant
...
If no context was retrieved, say so clearly.`
...
export const WebIntelligenceAgent = {
  async run(input: WebAgentInput): Promise<{ response: string; artifacts?: unknown[] }> {
    ...
    // Step 1: Determine if we need to search or scrape
    const needsSearch = !message.match(/^(what|who|when|where|why|how|tell me|explain)/i) ||
      message.match(/latest|current|today|news|2024|2025|2026/i)
    ...
    // Step 3: If a URL is in the message, scrape it
